...
from matplotlib.pyplot import plot, show, legend, figure, scatter, xticks,yticks,errorbar,ylim,xlim,xlabel,ylabel,title,stackplot,bar,subplots
...
Tref=(T1+T5+T9)/3
Iref=(I1+I5+I9)/3
Pin_ref=(Pin_1+Pin_5+Pin_9)/3
Nin_ref=(Nin_1+Nin_5+Nin_9)/3
Gref=(G1+G5+G9)/3
Theat=(T2+T6+T12)/3
Iheat=(I2+I6+I12)/3
Pin_heat=(Pin_2+Pin_6+Pin_12)/3
Nin_heat=(Nin_2+Nin_6+Nin_12)/3
Gheat=(G2+G6+G12)/3
...
pTref=(T3+T8+T10)/3
pIref=(I3+I8+I10)/3
pPin_ref=(Pin_3+Pin_8+Pin_10)/3
pNin_ref=(Nin_3+Nin_8+Nin_10)/3
pGref=(G3+G8+G10)/3
pTheat=(T4+T7+T11)/3
pIheat=(I4+I7+I11)/3
pPin_heat=(Pin_4+Pin_7+Pin_11)/3
pNin_heat=(Nin_4+Nin_7+Nin_11)/3
pGheat=(G4+G7+G11)/3
...
avg_T_ref,avg_I_ref,avg_Pin_ref,avg_Nin_ref,avg_G_ref=use_fnx(Tref,Iref,Pin_ref,Nin_ref,Gref)
avg_T_heat,avg_I_heat,avg_Pin_heat,avg_Nin_heat,avg_G_heat=use_fnx(Theat,Iheat,Pin_heat,Nin_heat,Gheat)

pavg_T_ref,pavg_I_ref,pavg_Pin_ref,pavg_Nin_ref,pavg_G_ref=use_fnx(pTref,pIref,pPin_ref,pNin_ref,pGref)
pavg_T_heat,pavg_I_heat,pavg_Pin_heat,pavg_Nin_heat,pavg_G_heat=use_fnx(pTheat,pIheat,pPin_heat,pNin_heat,pGheat)
...
Cbref,Cpref,Ceref,Csref,Cnsref,NPref,PCref,NCref=mod_macro_stoich(avg_I_ref,avg_Pin_ref,avg_Nin_ref,avg_T_ref,avg_G_ref)
Cbheat,Cpheat,Ceheat,Csheat,Cnsheat,NPheat,PCheat,NCheat=mod_macro_stoich(avg_I_heat,avg_Pin_heat,avg_Nin_heat,avg_T_heat,avg_G_heat)

pCbref,pCpref,pCeref,pCsref,pCnsref,pNPref,pPCref,pNCref=mod_macro_stoich2(pavg_I_ref,pavg_Pin_ref,pavg_Nin_ref,pavg_T_ref,pavg_G_ref)
pCbheat,pCpheat,pCeheat,pCsheat,pCnsheat,pNPheat,pPCheat,pNCheat=mod_macro_stoich2(pavg_I_heat,pavg_Pin_heat,pavg_Nin_heat,pavg_T_heat,pavg_G_heat)
...
nsto=Cnsheat-Cnsref
ess=Ceheat-Ceref
t=avg_T_heat-avg_T_ref


pbio=pCbheat-pCbref
psto=pCsheat-pCsref
ppho=pCpheat-pCpref
pnsto=pCnsheat-pCnsref
pess=pCeheat-pCeref
pt=pavg_T_heat-pavg_T_ref
...
plot(days,bio,label='Bio', color='#44AA99',linewidth=2)
plot(days,sto,label='C.Sto', color='#DDCC77',linewidth=2)
plot(days,pho,label='Pho', color='#CC6677',linewidth=2)
plot(days,t,label='Temp',color='black',linestyle='dashed',linewidth=2)
plot(z,zz,color='black',linewidth=2)
ylabel('C Allocation Anomoly (%)',fontsize=18)
xlabel('Day of Experiment',fontsize=18)
title('Eukaryotic Tanks',fontsize=20)
xlim(0,15)
legend(loc='upper right',fontsize='large',frameon=False)
ylim(top=30)
xticks(fontsize=16)
yticks(fontsize=16)
...
plot(days,pbio,label='Bio', color='#44AA99',linewidth=2)
plot(days,psto,label='C.Sto', color='#DDCC77',linewidth=2)
plot(days,ppho,label='Pho', color='#CC6677',linewidth=2)
plot(days,pt,label='Temp',color='black',linestyle='dashed',linewidth=2)
plot(z,zz,color='black',linewidth=2)
ylabel('C Allocation Anomoly (%)',fontsize=18)
xlabel('Day of Experiment',fontsize=18)
